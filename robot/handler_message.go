...
func (c MsgHandler) HandleMessage(packets *list.List) {
	for ele := packets.Front(); ele != nil; ele = ele.Next() {
		...
		if message.BizType == "into" {
			...
			services.GetMessageRepositoryInstance().InsertMessage(messageString)
			return
		}
		...
		services.GetMessageRepositoryInstance().InsertMessage(_messageString)
